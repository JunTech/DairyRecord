...
![1530555871804](1530555871804.png)

我们解压缩，放到工作目录中：

![1530367369490](1530367369490.png)



然后在Intellij idea中导入新的工程：

![1530367589197](1530367589197.png)

选中我们的工程：

 ![1530367781173](1530367781173.png)
...
![1530368695265](1530368695265.png)

我们只需要打开终端，进入项目目录，输入：`npm install`命令，即可安装这些依赖。

![1530374769782](1530374769782.png)

大概需要几分钟。

**如果安装过程出现以下问题**：

![1530374827792](1530374827792.png)
...
![1540706914029](1540706914029.png)

在package.json文件中有scripts启动脚本配置，可以输入命令：`npm run dev`或者`npm start`

![1530374954209](1530374954209.png)

发现默认的端口是9001。访问：http://localhost:9001

会自动进行跳转：

![1525958950616](1530375152204.png)



## 1.4.目录结构

 ![1525962755237](1525962755237.png)
...
![1525964023585](1525964023585.png)
...
![1543399927909](1543399927909.png)
...
![1525960652724](1525960652724.png)
...
![1530555978248](1530555978248.png)
...
![1525960312939](1525960312939.png)



我们重点关注`UI components`即可，里面有大量的UI组件，我们要用的时候再查看，不用现在学习，先看下有什么：

 ![1525961862771](1525961862771.png)

 ![1525961875288](1525961875288.png)
...
![1530380040278](1530380040278.png)

一个典型的三块布局。包含左，上，中三部分：

 ![1525965779366](1525965779366.png)
...
![1530556073565](1530556073565.png)

解压，运行exe文件，效果：

![1530382550630](1530382550630.png)
...
![1530382601757](1530382601757.png)

OK！

通过域名访问：

![1530383586463](1530383586463.png)

原因：我们配置了项目访问的路径，虽然manage.leyou.com映射的ip也是127.0.0.1，但是webpack会验证host是否符合配置。

![1530383612716](1530383612716.png)

在webpack.dev.conf.js中取消host验证：`disableHostCheck: true`

![1530383927461](1530383927461.png)

重新执行`npm run dev`，刷新浏览器：

![1532578408206](1532578408206.png)
...
### 3.4.1.什么是Nginx
 ...
 ![1526187409033](1526187409033.png)
...
![1526016663674](1526016663674.png)
...
![1530556268445](1530556268445.png)

解压后，目录结构：

![1530384792790](1530384792790.png)
...
示例：
 ...
 ![1526188831504](1526188831504.png)
...
启动过程会闪烁一下，启动成功后，任务管理器中会有两个nginx进程：
 ...
 ![1532579488518](1532579488518.png)
...
![1530385593052](1530385593052.png)

现在实现了域名访问网站了，中间的流程是怎样的呢？

![1526189945180](1526189945180.png)
...
![1530556389224](1530556389224.png)

我们先看商品分类表：

 ![1525999774439](1525999774439.png)
...
![1530427294644](1530427294644.png)
...
![1530427514123](1530427514123.png)
...
在`leyou-item-interface`中添加category实体类：
 ...
 ![1530444682670](1530444682670.png)
...
![1530445885707](1530445885707.png)
...
添加Controller：
 ...
 ![1530450599897](1530450599897.png)
...
一般service层我们会定义接口和实现类，不过这里我们就偷懒一下，直接写实现类了：
 ...
 ![1530450744567](1530450744567.png)
...
![1530455133230](1530455133230.png)

然后试试网关是否畅通：http://api.leyou.com/api/item/category/list

![1530455291468](1530455291468.png)

一切OK！

然后刷新后台管理页面查看：

![1530455437899](1530455437899.png)
...
![1530460311064](1530460311064.png)
...
结构：
 ...
 ![1530462650711](1530462650711.png)



重启测试，访问正常：

![1530463010927](1530463010927.png)
...
![1526021968036](1526021968036.png)

点击“品牌管理”菜单：

![1545221218980](1545221218980.png)
...
![1543066995215](1543066995215.png)
...
### 6.1.2.实体类
 ...
 ![1530541070271](1530541070271.png)
...
### 6.1.3.mapper
 ...
 ![1530541222679](1530541222679.png)
...
另外，这个PageResult以后可能在其它项目中也有需求，因此我们将其抽取到`leyou-common`中，提高复用性：
 ...
 ![1530543778541](1530543778541.png)
...
接下来，我们编写Controller
 ...
 ![1530549253999](1530549253999.png)
...
![1530549095150](1530549095150.png)
...
Vue官方推荐的ajax请求框架叫做：axios，看下demo：
 ...
 ![1526033988251](1526033988251.png)
...
而在我们的项目中，已经引入了axios，并且进行了简单的封装，在src下的http.js中：
 ...
 ![1526034150067](1526034150067.png)
...
- http.js中导入了config的配置，还记得吗？
  ...
  ![1526041205846](1526041205846.png)
...
![1543067111272](1543067111272.png)

网络监视：

 ![1526048143014](1526048143014.png)

resp到底都有那些数据，查看控制台结果：

![1526048275064](1526048275064.png)
...
![1526049643506](1526049643506.png)

成功实现分页功能：

![1526049720200](22.gif)
...
过滤字段对应的是search属性，我们只要监视这个属性即可:
 ...
 ![1526049939985](1526049939985.png)

查看网络请求：

 ![1526050032436](1526050032436.png)

页面结果：

![1530555740595](1530555740595.png)
