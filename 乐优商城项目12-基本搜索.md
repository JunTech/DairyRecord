...
![1532178218793](1532178218793.png)

![1532178276070](1532178276070.png)
...
![1532180648745](1532180648745.png)
...
再来看看页面中有什么数据：
 ...
 ![1526607712207](1526607712207.png) 
...
另外，页面还有过滤条件：
 ...
 ![1526608095471](1526608095471.png)
...
![1532213731039](1532213731039.png)
...
第二步，编写FeignClient
 ...
 ![1543416825258](1543416825258.png)
...
第一步：服务的提供方在`leyou-item-interface`中提供API接口，并编写接口声明：
 ...
 ![1543416889053](1543416889053.png)
...
第二步：在调用方`leyou-search`中编写FeignClient，但不要写方法声明了，直接继承`leyou-item-interface`提供的api接口：
 ...
 ![1543417084636](1543417084636.png)
...
在接口上按快捷键：`Ctrl + Shift + T`
 ...
 ![1532216103709](1532216103709.png)

 ![1532216169168](1532216169168.png)
...
![1532216884221](1532216884221.png)
...
### 1.4.1.创建GoodsRepository
 ...
 ![1543418137705](1543418137705.png)
...
我们新建一个测试类，在里面进行数据的操作：
 ...
 ![1543417536876](1543417536876.png)
...
![1532217819818](1532217819818.png)
...
![1532228358310](1532228358310.png)
...
![1526629923970](1526629923970.png)

当我们输入任何文本，点击搜索，就会跳转到搜索页`search.html`了：

并且将搜索关键字以请求参数携带过来：

![1532229236516](1532229236516.png)
...
![1532233280898](1532233280898.png)

并在leyou-gateway工程的Application.yml中添加网关映射：

![1532233247824](1532233247824.png)



刷新页面试试：

![1532233086523](1532233086523.png)
...
### 2.2.1.controller
 ...
 ![1543418199310](1543418199310.png)
...
### 2.2.2.service
 ...
 ![1543418304067](1543418304067.png)
...
![1532237344249](1532237344249.png)

![1532237401249](1532237401249.png)
...
![1532237986819](1532237986819.png)
...
![1532239032197](1532239032197.png)

在`loadData`的异步查询中，将结果赋值给`goodsList`：

![1532239117076](1532239117076.png)
...
我们先来实现sku的选择，才能去展示不同sku的数据。
 ...
 ![1526654252710](1526654252710.png)
...
![1532240220800](1532240220800.png)

我们在查询成功的回调函数中，对goods进行遍历，把skus转化成json对象集合，并添加一个selected属性保存被选中的sku：

![1532240609206](1532240609206.png)

![1532240586769](1532240586769.png)
...
![1532240706261](1532240706261.png)
...
![1526656197524](1526656197524.png)



刷新页面：

 ![1526656243166](1526656243166.png)
...
![1532242831006](1532242831006.png)

结果报错：

![1532242950035](1532242950035.png)

为啥？

因为在Vue范围内使用任何变量，都会默认去Vue实例中寻找，我们使用ly，但是Vue实例中没有这个变量。所以解决办法就是把ly记录到Vue实例：

![1532242983324](1532242983324.png)

然后刷新页面：

![1532243052100](1532243052100.png)
...
![1526656959487](1526656959487.png)
...
![1532243182104](1532243182104.png)

这段代码稍微改造一下，即可：

![1532243275078](1532243275078.png)
...
先看下页面关于分页部分的代码：
 ...
 ![1526692249371](1526692249371.png)
...
![1532243978471](1532243978471.png)
...
在我们返回的PageResult对象中，其实是有totalPage字段的：
  ...
  ![1526695144476](1526695144476.png)

我们在返回时，把这个值填上：

 ![1526695592422](1526695592422.png)

页面测试一下：

![1532244453375](1532244453375.png)
...
首先，把后台提供的数据保存在data中：
 ...
 ![1526695967230](1526695967230.png)



然后看下我们要实现的效果：

![1526695821870](1526695821870.png)
...
![1532246481241](1532246481241.png)
...
![1526698842013](1526698842013.png)
...
![1532248549662](1532248549662.png)
...
在页面商品列表的顶部，也有一个分页条：
 ...
 ![1526716212704](1526716212704.png)

我们把这一部分，也加上点击事件：

![1532248435097](1532248435097.png)
...
在搜索商品列表的顶部，有这么一部分内容：
 ...
 ![1526716565293](1526716565293.png)
...
因此，我们首先在`search`中记录这两个信息，因为created钩子函数会对search进行覆盖，因此我们在钩子函数中对这两个信息进行初始化即可：
 ...
 ![1526717586493](1526717586493.png)
...
可以看到，页面请求参数中已经有了排序字段了：
 ...
 ![1526718252315](1526718252315.png)
...
现在，我们的请求参数对象`SearchRequest`中，只有page、key两个字段。需要进行扩展：
 ...
 ![1526718448918](1526718448918.png)



然后在搜索业务逻辑中，添加排序条件：

![1526718637618](1526718637618.png)
...
![1526719415219](1526719415219.png)
